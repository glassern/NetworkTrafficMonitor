...
class TrafficMonitor:
    ...
    def handle_response(self, response):
        ...
        print(f"  统计: {self._stats['total']}个请求 | 成功:{self._stats['success']} 失败:{self._stats['fail']} | 平均耗时: {avg:.0f}ms")
    ...
    def start(self):
        ...
        with sync_playwright() as p:
            ...
            if self._stats["total"] > 0:
                print(f"\n本次监控统计:")
                print(f"   总请求数: {self._stats['total']}")
                print(f"   成功/失败: {self._stats['success']}/{self._stats['fail']}")
                print(f"   平均耗时: {self._stats['total_duration'] / self._stats['total']:.0f}ms")
