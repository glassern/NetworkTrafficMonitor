...
class TrafficMonitor:
    ...
    def __init__(self, target_url, output_file, filter_keywords=None,
        ...
        self._records = []
        self._stats = {"total": 0, "success": 0, "fail": 0, "total_duration": 0.0}
    ...
    def handle_response(self, response):
        ...
        avg = self._stats["total_duration"] / self._stats["total"]
        print(f"[{record['Method']}] {record['URL']} -> {record['Status']} ({duration}ms)")
        print(f"  Stats: {self._stats['total']} requests | OK:{self._stats['success']} FAIL:{self._stats['fail']} | Avg: {avg:.0f}ms")
    ...
    def start(self):
        ...
        with sync_playwright() as p:
            ...
            if self._stats["total"] > 0:
                print(f"\nSession Stats:")
                print(f"   Total requests: {self._stats['total']}")
                print(f"   Success/Fail: {self._stats['success']}/{self._stats['fail']}")
                print(f"   Avg duration: {self._stats['total_duration'] / self._stats['total']:.0f}ms")
    ...
    def save_curl_commands(self):
        ...
        lines = [
            ...
            f"# Total: {len(self._records)} requests",
            ""
        ]
